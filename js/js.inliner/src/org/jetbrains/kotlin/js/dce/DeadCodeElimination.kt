...
class DeadCodeElimination(val root: JsNode) {
    ...
    fun apply() {
        ...
        for ((fqn, expressions) in fqnToExpressions.toList()) {
            for (altFqn in fqn.collectRootFqNames()) {
                fqnToExpressions.getOrPut(altFqn, ::mutableSetOf) += expressions
            }
        }
        ...
        for ((fqn, fqnDeps) in dependencies.toList()) {
            ...
            for (altFqn in fqn.collectRootFqNames()) {
                dependencies.getOrPut(altFqn, ::mutableSetOf) += fqnDeps.flatMap { it.collectRootFqNames() }
            }
        }
    ...
    private val visitor = object : JsVisitor() {
        ...
        override fun visitExpressionStatement(x: JsExpressionStatement) {
            ...
            if (expression is JsBinaryOperation) {
                ...
                expression.name?.let { nodes[it] }?.let {
                    ...
                    fqnMap[x] = fqn
                    fqnToExpressions.getOrPut(fqn, ::mutableSetOf) += expression
                }
            }
        ...
        private fun processAssignment(lhs: JsExpression, rhs: JsExpression): Fqn? {
            ...
            if (leftFqn != null && !leftFqn.isRoot) {
                addDependency(leftFqn.parent(), leftFqn)
            }
            ...
            if (leftFqn != null && rightFqn != null) {
                ...
                if (rhs is JsInvocation) {
                    ...
                    fqnToExpressions.getOrPut(leftFqn, ::mutableSetOf) += rhs
                    return leftFqn
                }
        ...
        private fun handleObjectCreate(target: Fqn, arg: JsExpression?) {
            ...
            fqnToExpressions.getOrPut(target, ::mutableSetOf) += arg
        }
        ...
        private fun addDependency(from: Fqn, to: Fqn) {
            dependencies.getOrPut(from, ::mutableSetOf) += to
        }
    }
    ...
    private val usageFinder = object : RecursiveJsVisitor() {
        ...
        private fun useAncestors(fqn: Fqn) {
            if (fqn in fqnsWithSideEffects && fqn !in usedFqns) {
                ...
                //dependencies[fqn]?.forEach { useAncestors(it) }
                if (!fqn.isRoot) {
    ...
    private fun extractFqn(expression: JsExpression): Fqn? {
        return if (expression is JsNameRef) {
            val path = mutableListOf<String>()
            val rootNode = extractRootNode(expression, path)
            if (rootNode != null) Fqn(rootNode, path.asReversed()) else null
        }
        else {
            null
        }
    ...
    private fun extractRootNode(nameRef: JsNameRef, pathConsumer: MutableList<String>): FqnRoot? {
        val qualifier = nameRef.qualifier
        return when (qualifier) {
            null -> {
                nameRef.name?.let { nodes[it] } ?: let {
                    pathConsumer += nameRef.ident
                    globalScope
                }
            }
            is JsNameRef -> {
